...
코드베이스에서 AI 기본 패턴(Inter 폰트, 보라 그래디언트, 3열 카드 그리드, 과도한 border-radius)을 감지하고, **브라우저 스크린샷으로 실제 렌더링을 시각적으로 검수**한 뒤, 점수화된 감사 리포트와 프로젝트 전용 디자인 시스템을 생성하여 적용합니다. 필요하면 스택 마이그레이션까지 제안합니다.
...
스킬에 포함된 무의존 Node 스캐너로 코드 레벨 신호를 먼저 확인할 수 있습니다.

```bash
node skills/ui-refiner/scripts/audit-ui-patterns.mjs /path/to/project
node skills/ui-refiner/scripts/audit-ui-patterns.mjs /path/to/project --json
```
...
| **Phase 1** · 감사 | 스캐너 + `AUDIT.md` 점수표로 AI 기본 패턴, 스택 제약, 접근성 리스크 평가 |
| **Phase 2** · 디자인 시스템 | 콘텐츠·청중 기반 확립 → 5가지 디자인 원칙 → 토큰 시스템 → 2-pass 자기 비판 |
| **Phase 3** · 적용 | 필요 시 스택 마이그레이션 후 섹션별 교체 (모션·배경 포함) |
| **Phase 4** · 시각적 검증 | 수정 후 재스크린샷으로 시각·반응형·접근성 개선 확인 *(선택)* |

핵심은 **점수화된 감사 + 2-pass 자기 비판**입니다. 먼저 `AUDIT.md` 기준으로 심각도를 고정하고, 만든 디자인이 "다른 업종 사이트에도 그대로 쓰일 수 있는가?"를 묻습니다. 그다음 흔한 AI 기본값 3가지(크림+세리프+테라코타 / 검정+네온 / 3단 컬럼)와 대조해 걸러냅니다.
...
- focus-visible, reduced-motion, empty/error/loading 상태 누락 → 기본 접근성 회복
...
├── skills/
│   └── ui-refiner/
│       ├── SKILL.md         # 스킬 본문 (고정 워크플로우)
│       ├── AUDIT.md         # 점수표·심각도·스택/접근성 감사 기준
│       ├── OUTPUTS.md       # 감사·디자인 시스템·검증 리포트 템플릿
│       ├── REFERENCE.md     # 패턴 카탈로그·폰트·마이그레이션 가이드
│       └── scripts/
│           └── audit-ui-patterns.mjs
├── CHANGELOG.md
└── README.md
```
